...
@dataclass
class Motion2VideoPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]


class Motion2VideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        obj_feat = self.obj_encoder(control_objs.to(self.obj_encoder.device))
        cam_feat = self.cam_encoder(control_cams.to(self.cam_encoder.device))

        if is_obj and not is_cam:
            final_feat = obj_feat
        elif not is_obj and is_cam:
            final_feat = cam_feat
        elif is_obj and is_cam:
            final_feat = self.fusion_module(obj_feat, cam_feat)
        else:
            final_feat = None
        ...
        return Motion2VideoPipelineOutput(videos=images)
